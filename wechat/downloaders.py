...
class SplinterDownloaderBackend(object):
    headers = [
        {
    ...
    def get_browser(self, proxy):
        ...
        if proxy.is_valid():
            ...
            kwargs['proxy'] = ff_proxy
        if settings.HEADLESS:
            kwargs['headless'] = True
        browser = Browser('chrome', options=options, **kwargs)
        return browser
    ...
    def visit_wechat_index(self, wechatid):
        ...
        browser = self.browser
        browser.visit("http://weixin.sogou.com/")
        print(browser.title)
        element_querybox = browser.find_by_name('query').fill(wechatid)
        # element_querybox.send_keys(wechatid, Keys.ARROW_DOWN)
        element_search_btn = browser.find_by_xpath("//input[@value='搜公众号']")
    ...
    def visit_wechat_index_keyword(self, word):
        ...
        browser.visit("http://weixin.sogou.com/")
        print(browser.title)
        element_querybox = browser.find_by_name('query').fill(word)
        element_search_btn = browser.find_by_xpath("//input[@value='搜文章']")
        element_search_btn.click()
        time.sleep(3)
        print(browser.title)
    ...
    def visit_wechat_topic_list(self, wechatid):
        ...
        element_wechat = browser.find_by_xpath("//div[@class='txt-box']/p[@class='info']/label")
        element_wechat_title = browser.find_by_xpath("//div[@class='txt-box']/p[@class='tit']/a")
        if element_wechat and element_wechat.text == wechatid:
            element_wechat_title.click()
            time.sleep(3)
            # 切到当前的文章列表页窗口
            new_handler = browser.driver.window_handles[-1]
            browser.driver.switch_to.window(new_handler)
            time.sleep(3)
            return True
        else:
            return False
    ...
    def download_wechat_topics(self, wechat_id, process_topic):
        ...
        body = browser.driver.execute_script(js)
    ...
    def download_wechat_keyword_topics(self, word, process_topic):
        ...
        while 1:
            ...
            try:
                button_next = browser.find_by_xpath("//a[@id='sogou_next']")
                if button_next:
                    button_next.click()
                    time.sleep(2)
                else:
